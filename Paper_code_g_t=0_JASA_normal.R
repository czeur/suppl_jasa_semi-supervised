...
for(le in 1:repl){
  ...
  g_t=-0.3
  my_mu2 <- c(0,0,0)                                
  my_Sigma2 <- matrix(c(1,s_,s_,s_,1,r,s_,r,1),ncol = 3)
  hh1<-mvrnorm(n = sz, mu = my_mu2, Sigma = my_Sigma2)
  zz=matrix(NA,sz,3)
  for (q in 1:sz) {if (hh1[q,1]>0 && hh1[q,2]>0) {zz[q,]=hh1[q,]}}
  hh=na.omit(zz)
  pre1_x=hh[1:n,1]
  #standard normal 
  pre2_x=(2*pnorm(pre1_x))-1
  
  if(g_t==0) x1=-log(1-pre2_x) else x1=(1-((1-pre2_x)^(-g_t)))/-g_t 
