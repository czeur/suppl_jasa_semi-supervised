...
for (l in 1:length(a_vec)){
  if(a_vec[l]==0) {for (re in 1:repl){
    ...
    ###########3d###########
    R_a=(R1_y-(((-gamma_1)/(gamma_1+1))*((((2*gamma_1)+1)*R_s_y_int)-R_t_y)))
    R_b=(R1_z-(((-gamma_1)/(gamma_1+1))*((((2*gamma_1)+1)*R_s_z_int)-R_t_z)))
    R_ab=R_y_z
    c2=(1+gamma_1)*((R_a-(R_ab*R_b))/(1-(R_ab^2)))
    c3=(1+gamma_1)*((R_b-(R_ab*R_a))/(1-(R_ab^2)))
    ...
    ###########2d###########
    R_s=(((3*gamma_imp_2d)-1)*R_t_y)+(gamma_imp_3d*R_lnt_y)-((((2*gamma_imp_2d)+1)^2)*R_s_y_int_2d)+((2*(gamma_imp_2d+2))*R1_y)
    sigma_imp_2d=sigma_1*(1-((R_s/2)*((sigma_2)-1)))
    
    ###########3d###########
    z12=((((3*gamma_imp_3d)-1)*R_t_y)+(gamma_imp_3d*R_lnt_y)-((((2*gamma_imp_3d)+1)^2)*R_s_y_int_3d)+(2*(gamma_imp_3d+2)*R1_y)) 
    z13=((((3*gamma_imp_3d)-1)*R_t_z)+(gamma_imp_3d*R_lnt_z)-((((2*gamma_imp_3d)+1)^2)*R_s_z_int_3d)+(2*(gamma_imp_3d+2)*R1_z))
    ...
    Q12_33=(sigma_imp_3d*((((k/(n*p))^gamma_imp_3d)-1)/gamma_imp_3d))
  ...
  for (re in 1:repl){
    ...
    indicator4x=matrix(NA,n,1)
    for(i in 1:n){ if(x[i,re]>x_sort[n-k] && y[i,re]>y_sort[n-k]){indicator4x[i]=(log((n-x_rank[i]+1)/k))} else{indicator4x[i]=0}}
    R_s_x=(-(sum(indicator4x)/k)) 
    
    indicator4=matrix(NA,n,1)
    for(i in 1:n){ if(x[i,re]>x_sort[n-k] && y[i,re]>y_sort[n-k]){indicator4[i]=(log((n-y1_rank[i]+1)/k))} else{indicator4[i]=0}}
    R_t_y=(-(sum(indicator4)/k)) 
    indicator4ttt=matrix(NA,n,1)
    for(i in 1:n){ if(x[i,re]>x_sort[n-k] && z[i,re]>z_sort[n-k]){indicator4ttt[i]=(log((n-z1_rank[i]+1)/k))} else{indicator4ttt[i]=0}}
    R_t_z=(-(sum(indicator4ttt)/k)) #R_xz(1,t)/t
    ...
    ############3d#############
    R_a=(R1_y-(((a-gamma_1)/(gamma_1+a+1))*((((2*gamma_1)+1)*R_s_y_int)-(((2*a)+1)*R_t_y_int))))
    R_b=(R1_z-(((a-gamma_1)/(gamma_1+a+1))*((((2*gamma_1)+1)*R_s_z_int)-(((2*a)+1)*R_t_z_int))))
    R_ab=R_y_z
    c2=((1+gamma_1)/(1+a))*((R_a-(R_ab*R_b))/(1-(R_ab^2)))
    c3=((1+gamma_1)/(1+a))*((R_b-(R_ab*R_a))/(1-(R_ab^2)))  
    ...
    indicator1t=matrix(NA,n,1)
    for(i in 1:n){ if(x[i,re]>x_sort[n-k] && y[i,re]>y_sort[n-k]){indicator1t[i]=((n-x_rank[i]+1)/k)^gamma_imp_2d} else{indicator1t[i]=0}}
    R1_intgt_2d=(1/gamma_imp_2d)*(R1_y-(sum(indicator1t)/k)) #R_xy(s,1)/s^{1-\gamma_tilde_2d}
    
    #########################################improved estimator sigma###########################################
    #############2d#############
    g1t=(gamma_imp_2d+1)
    g3t=((2*gamma_imp_2d)+1)
    g4t=((2*a)+1)
    g5t=(gamma_imp_2d+a+1)
    g2=(1+a)
    R_1_cont=(((g1t^2)*(g2^2))-(g1t^2)-(g2^2)+1)/(gamma_imp_2d*a)*R1_y
    R_s_cont=-((a*(gamma_imp_2d+1))/gamma_imp_2d)*R_s_x
    R_t_cont=-((gamma_imp_2d*(a+1))/a)*R_t_y
    R1_intg_cont=((g1t*g2)/(gamma_imp_2d*a))*(((g3t*g1t)/g2)-(g1t*g2*g3t)-g3t+((g1t*g2*g3t*g4t)/g5t))*R1_intgt_2d
    R2_intg_cont=((g1t*g2)/(gamma_imp_2d*a))*(((g4t*g2)/g1t)-(g1t*g2*g4t)-g4t+((g1t*g2*g3t*g4t)/g5t))*R_t_y_int
    R_st=(R_1_cont+R_s_cont+R_t_cont+R1_intg_cont+R2_intg_cont)
    ct=(R_st/(1+(g2^2)))
    sigma_imp_2d=sigma_1*(1-(ct*((sigma_2/(n/k)^a)-1)))
    #############3d#############
    indicator1td=matrix(NA,n,1)
    for(i in 1:n){ if(x[i,re]>x_sort[n-k] && y[i,re]>y_sort[n-k]){indicator1td[i]=((n-x_rank[i]+1)/k)^gamma_imp_3d} else{indicator1td[i]=0}}
    R_s_yt_int_3d=(1/gamma_imp_3d)*(R1_y-(sum(indicator1td)/k)) #R_xy(s,1)/s^{1-\gamma_tilde_3d}
    indicator2td=matrix(NA,n,1)
    for(i in 1:n){ if(x[i,re]>x_sort[n-k] && z[i,re]>z_sort[n-k]){indicator2td[i]=((n-x_rank[i]+1)/k)^gamma_imp_3d} else{indicator2td[i]=0}}
    R_s_zt_int_3d=(1/gamma_imp_3d)*(R1_z-(sum(indicator2td)/k)) #R_xz(s,1)/s^{1-\gamma_tilde_3d}
    g1t=(gamma_imp_3d+1)
    g3t=((2*gamma_imp_3d)+1)
    g4t=((2*a)+1)
    g5t=(gamma_imp_3d+a+1)
    R_1_cont=((((g1t^2)*(g2^2))-(g1t^2)-(g2^2)+1)/(gamma_imp_3d*a))*R1_y
    R_s_cont=-((a*(gamma_imp_3d+1))/gamma_imp_3d)*R_s_xy
    R_t_cont=-((gamma_imp_3d*(a+1))/a)*R_t_y
    R1_intg_cont=((g1t*g2)/(gamma_imp_3d*a))*(((g3t*g1t)/g2)-(g1t*g2*g3t)-g3t+((g1t*g2*g3t*g4t)/g5t))*R_s_yt_int_3d
    R2_intg_cont=((g1t*g2)/(gamma_imp_3d*a))*(((g4t*g2)/g1t)-(g1t*g2*g4t)-g4t+((g1t*g2*g3t*g4t)/g5t))*R_t_y_int
    z12=(R_1_cont+R_s_cont+R_t_cont+R1_intg_cont+R2_intg_cont)
    R_1_contz=(((g1t^2)*(g2^2))-(g1t^2)-(g2^2)+1)/(gamma_imp_3d*a)*R1_z
    R_s_contz=-((a*(gamma_imp_3d+1))/gamma_imp_3d)*R_s_xz
    R_t_contz=-((gamma_imp_3d*(a+1))/a)*R_t_z
    R1_intg_contz=((g1t*g2)/(gamma_imp_3d*a))*(((g3t*g1t)/g2)-(g1t*g2*g3t)-g3t+((g1t*g2*g3t*g4t)/g5t))*R_s_zt_int_3d
    R2_intg_contz=((g1t*g2)/(gamma_imp_3d*a))*(((g4t*g2)/g1t)-(g1t*g2*g4t)-g4t+((g1t*g2*g3t*g4t)/g5t))*R_t_z_int
    z13=(R_1_contz+R_s_contz+R_t_contz+R1_intg_contz+R2_intg_contz)
    z23=(((a^2)+(4*a)+4)*R_y_z)-((a+1)*(R_s_yz+R_t_yz))
    g6= (1+((1+a)^2))
    con1=((g6*z12)-(z13*z23))/((g6^2)-(z23^2))
    con2=((g6*z13)-(z12*z23))/((g6^2)-(z23^2))
    #sigma_tilde_3d=sigma_1/(1+(con1*((sigma_2/(n/k)^a)-1)+(con2*((sigma_3/(n/k)^a)-1))))
